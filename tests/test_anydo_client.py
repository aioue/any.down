...
class TestAnyDoClient(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Patch _load_session to prevent actual file loading during init
    ...
    def test_login_failure(self):
        """Test login failure when email check fails."""
        mock_response = Mock()
        mock_response.status_code = 200
    ...
    def test_handle_2fa_interactive_empty_code(self):
        """Test interactive 2FA with empty code then valid code."""
        with patch.object(self.client, '_trigger_2fa_email', return_value=True):
    ...
    def test_verify_2fa_code_success(self):
        ...
        # Set up temp credentials
        self.client._temp_email = "test@example.com"
        self.client._temp_password = "password123"
    ...
    def test_test_session_failure(self):
        """Test session validity testing when session is invalid."""
        mock_response = Mock()
    ...
    def test_save_markdown_tasks(self):
        ...
        if result:  # Type guard for mypy
            ...
            self.assertIn('<span style="color: #666; font-style: italic;">Test note</span>', content)
